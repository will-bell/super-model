...
from twolink_manipulator.common import distance
...
class Potential:
    ...
    def __init__(self, minimum: np.ndarray, zeta: float = .1, quadratic_radius: float = 2.5):
    ...
    def evaluate_potential_gradient(self, point: np.ndarray) -> np.ndarray:
        d2goal = distance(point, self._minimum)
        if d2goal <= self._quadratic_radius:
            value = self._zeta * (point - self._minimum)
        else:
            value = self._quadratic_radius * self._zeta * (point - self._minimum) / d2goal
